...
```bash
dorothy_personel$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
dorothy_personel$ sam logs -n HelloWorldFunction --stack-name "dorothy_personel" --tail
```
...
```bash
dorothy_personel$ pip install pytest pytest-mock --user
dorothy_personel$ python -m pytest tests/ -v
```
